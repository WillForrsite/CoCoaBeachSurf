...
RefundsOnReportDate AS (
    SELECT 
        p.pmtinvID AS RsrvtnID,
        SUM(p.pmtAmt) AS RefundAmt
    FROM tblARPayments p
    WHERE CAST(p.pmtCrtdDtTm AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS DATE) = @reportdate
      AND p.pmtType = 'RF'
    GROUP BY p.pmtinvID
),

-- Refunds joined to original trip metadata
RefundDetails AS (
    SELECT DISTINCT
        r.RsrvtnID,
        vts.REVGRP,
        ...
        description = CONCAT(
            'REFUND ',
            CASE 
                WHEN vts.price / NULLIF(vts.trpPssngrs, 0) = 25 THEN 'RoundTrip '
                WHEN vts.price / NULLIF(vts.trpPssngrs, 0) = 28 THEN 'OneWay '
                ELSE 'OneWay '
            END,
            'Shuttle ',
            vts.REVGRP,
    ...
    FROM RefundsOnReportDate r
    LEFT JOIN vw_FPTripInvoiceSummary vts
        ON vts.RsrvtnID = r.RsrvtnID
),

-- Regular revenue rows for the report date
RevenueDetails AS (
    SELECT
        vts.RsrvtnID,
        vts.REVGRP,
    ...
    WHERE vts.trpPckDate = @reportdate
      AND vts.trpStatus <> 'e'
      AND vts.RsrvtnSrcClean NOT IN ('Hotel')
),
CombinedRevenue as (
-- Combine both sets and aggregate
SELECT
    description,
    transactions = COUNT_BIG(RsrvtnID),
    pax = SUM(ISNULL(trpPssngrs, 0)),
    amount = SUM(
        CASE 
            WHEN RefundAmt <> 0 THEN RefundAmt
            ELSE ISNULL(price, 0)
        END
    )
FROM (
    SELECT * FROM RevenueDetails
    UNION ALL
...
CurrentDayRevenueFinal AS (
    SELECT
        description = concat(isnull(rf.refund,0) ,
                    CASE 
    ...
    FROM vw_FPTripInvoiceSummary vts
    LEFT JOIN (SELECT pmtinvID,Refund = sum(pmtAmt) FROM tblARPayments 
                where cast(pmtCrtdDtTm AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' as Date) = @reportDAte
                  and pmtType = 'RF'
               Group by pmtinvID) rf on rf.pmtinvID = vts.RsrvtnID
                
    WHERE RsrvtnSrcClean NOT IN ('Hotel')
      AND trpStatus <> 'e'
      AND trpPckDate = @ReportDate
      --AND trpPckDate = @ReportDate
    GROUP BY concat(isnull(rf.refund,0) ,
                    CASE 
                    WHEN price / NULLIF(trpPssngrs,0) = 25 THEN 'RoundTrip '
                    WHEN price / NULLIF(trpPssngrs,0) = 28 THEN 'OneWay '
                    ELSE 'OneWay ' 
...
SELECT @ReportJson = (
  SELECT
    ...
    -- Arrays
    (SELECT * FROM CombinedRevenue FOR JSON PATH, INCLUDE_NULL_VALUES) AS currentDayRevenue,
