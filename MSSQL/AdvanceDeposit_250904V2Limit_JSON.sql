...
DailyDeposits AS (
    ...
    --  and cast(pmtCrtdDttm AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' as date) > '2025-09-04'
    --order BY CAST(pmtCrtdDtTm AS date)
    --GROUP BY CAST(pmtCrtdDtTm AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS date)    
...
--        CAST(RsrvtnDt AS date) AS pmtDate
--    FROM AllDates ad
--    left join vw_FPTripInvoiceSummary vts on CAST(RsrvtnDt AS date) = ad.ReportDate   
--    WHERE vts.RsrvtnSrcClean NOT IN ('Hotel')  -- AD rule
----      AND vts.trpStatus IN ('c','p','e')
----      AND REVGRP NOT LIKE '%Same Day%'
--      AND ad.ReportDate = @ReportDate
----      AND CAST(vts.trpPckDate as date) > @ReportDate
--),
CurrentDayCancel AS (
    SELECT
...
--        REVGRP,
--        pmtDate,
--        SUM(trpPssngrs) AS totalPax,
--        SUM(price)      AS totalPrice
--    FROM CurrentDayRevenue
--    GROUP BY REVGRP,pmtDate
--),
...
--    REVGRP = CONCAT(
--        CASE
--            WHEN totalPrice / NULLIF(totalPax, 0) = 25 THEN 'RoundTrip'
--            WHEN totalPrice / NULLIF(totalPax, 0) = 28 THEN 'OneWay'
--            ELSE 'OneWay'
--        END,
--        REVGRP  -- << your prefix here
--    ),
--    trpPckDate = pmtDate,
--    trpPssngrs = totalPax,
--    price      = totalPrice
...
--    SELECT
--        REVGRP = concat(CASE
--                     WHEN price / NULLIF(sum(totalPax),0) = 25 THEN 'RoundTrip '
--                     WHEN price / NULLIF(sum(totalPax),0) = 28 THEN 'OneWay '
--                     ELSE 'OneWay '
--                 END, REVGRP),
--        trpPckDate = pmtDate,
--        trpPssngrs = sum(trpPssngrs),
--        price = sum(price)
--    FROM CurrentDayRevenue    
--    group by concat(CASE
--                     WHEN price / NULLIF(sum(price),0) = 25 THEN 'RoundTrip '
--                     WHEN price / NULLIF(sum(price),0) = 28 THEN 'OneWay '
--                     ELSE 'OneWay '
--                 END, REVGRP),
--                 pmtDate
    --SELECT
...
--        REVGRP,
--        transactions = COUNT_BIG(*),
--        pax          = SUM(ISNULL(pax,0)),
--        totalPrice   = SUM(ISNULL(totalPrice,0))
...
SELECT @ReportJson = (
  SELECT
    ...
    --        pax,          
    --        totalPrice   
    --   FROM CurrentDayRevenueFinal  FOR JSON PATH, INCLUDE_NULL_VALUES) AS preArrangedSales,
