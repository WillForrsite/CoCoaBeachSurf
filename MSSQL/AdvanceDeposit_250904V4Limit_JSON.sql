...
AllDates AS (
    ...
    UNION
    SELECT DISTINCT CAST(case
                         when pmtUpdt is not null then pmtUpdt AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time'
                         else pmtCrtdDtTm AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time'
                          end AS date)
    FROM tblARpayments
